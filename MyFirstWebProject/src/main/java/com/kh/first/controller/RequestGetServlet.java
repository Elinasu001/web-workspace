...
public class RequestGetServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 서블릿 생명주기 / 싱그톤 패턴으로 객체 하나만을 사용 / 톰캣이 멀티스레드 처리(스레드플)
		 */
		...
		/*
		 * request 의 parameter영역으로부터 전달된 데이터 값을 뽑아내는 방법
		 * 
		 ...
		 * => 하나의 key값으로 여러 개의 value들을 받아야 할 경우 ex) checkBox (여러가지 있음)
		 */
		...
		// 값의 정보 : "셀 수 없음"
		// 사용자가 정상적인 값을 입력 : "홍길동" / 아무것도 입력 안하고 요청 했을 경우 : ""(텍스트상자가 비어있을 경우 빈문자열) / name 이라는 키값을 지울 경우 : null
		
		String gender = request.getParameter("gender");
		System.out.println(gender);
		// "남" / "여" / "선택 안 함 " / null(기본적으로 input checked 해주는 추가 적인 절차 필요)
		...
		//int realHeight = (int)height; // 강제형변환
		...
		// 			=>  사용자 요청 처리 후에 처리 할 수 없는 문제 발생 => 예외 처리 안하여 내부 서버 오류 
		...
		// 3단계. 요청 처리 ( DB와의 상호 작용 == JDBC/MyBatis)
		// 보통의 흐름 : Controller 에서 Service의 메소드를 호출하면서 값을 전달
		...
		pw.println("<html>");
			...
			pw.println("<body>");
				...
				pw.printf("<span id='city'>%s</span>에 삽니다. <br><br>", city);
				
				pw.printf("<span id='height'>%.1f</span>cm이고  <br>", height);
				
				
				pw.print("성별은 ");			// gender 앞에가면 nullpointexception 주의
				if(gender == null || "선택안함".equals(gender)) {
					pw.println("선택을 안했습니다. <br><br>");
				} else if(gender.equals("남")) {
					pw.println("<span id='gender'>남자</span>입니다. <br><br>");
				} else {
					pw.println("<span id='gender'>여자</span>입니다. <br><br>");
				}
